...
TOKENS = int(sys.argv[3]) if len(sys.argv) > 3 else 120     # ~100–150 words
STRIDE = int(sys.argv[4]) if len(sys.argv) > 4 else 100     # overlap ~20%
...
with IN.open() as fin, OUT.open("w") as fout:
    for line in fin:
        ...
        for j, seg in enumerate(chunk_words(words, TOKENS, STRIDE)):
            passage = f"{title}. " + " ".join(seg) if title else " ".join(seg)
            out = {
                "paper_id": pid, "title": title, "chunk_id": j,
                "passage": passage, "category": rec.get("category")
            }
